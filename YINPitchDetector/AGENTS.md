...
- Keep functions focused, avoid unnecessary complexity, and document non‑obvious DSP math inline.

## Testing Guidelines
- Framework: Rust built‑in `#[test]` colocated near logic in `src/`.
- Tolerance: use DSP‑friendly checks (e.g., frequency within ±5%).
- Naming: group with `mod tests { ... }`; future integration tests will live under `tests/`.
- Run: `cargo test`; when fixing bugs, include a failing repro test.

## Commit & Pull Request Guidelines
- Commits: imperative, concise subject ≤72 chars; add scope when helpful (e.g., `rust:`, `juce:`). Include rationale, trade‑offs, and testing notes.
- PRs: clear description, linked issues, repro steps; screenshots/GIFs for UI changes.
- Ensure code is formatted, lints are clean, and tests pass before requesting review.

## Security & Configuration Tips
- Do not commit secrets or local build artifacts.
- macOS: microphone permission is required for real‑time audio; note platform quirks in PRs.
- Treat `JUCE/` as vendor code; modify only when updating the vendor.
